...
The strict job owns its session displays and cannot borrow legacy `DISPLAY=:99`. Ubuntu hosts with `kernel.apparmor_restrict_unprivileged_userns=1` must receive an administrator-approved Bubblewrap profile or enable the prerequisite; the disposable GitHub runner sets it to `0` explicitly before probing Bubblewrap. Production preflight refuses hosts where the capability remains denied. Required-gate mode fails on missing prerequisites, capability denial, skipped acceptance cases, or zero collected gate tests. Local normal test runs may leave engine tests opt-in. Upload only bounded allowlisted synthetic diagnostics, sanitized manifests, and fixture frames; exclude tokens, publication/control directories, and profile payloads.
